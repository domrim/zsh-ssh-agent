...
function _start_agent() {
  ...
  zstyle -t :plugins:ssh-agent quiet || echo >&2 "Starting ssh-agent ..."
...
function _add_identities() {
  ...
  local args
  zstyle -a :plugins:ssh-agent ssh-add-args args

  # if ssh-agent quiet mode, pass -q to ssh-add
  zstyle -t :plugins:ssh-agent quiet && args=(-q $args)

  # use user specified helper to ask for password (ksshaskpass, etc)
  local helper
  zstyle -s :plugins:ssh-agent helper helper
...
if zstyle -t :plugins:ssh-agent agent-forwarding \
...
if ! zstyle -t :plugins:ssh-agent lazy; then
  _add_identities
fi
